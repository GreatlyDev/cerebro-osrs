...
@pytest.mark.asyncio
async def test_chat_can_identify_what_makes_account_feel_more_premium(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"PRM{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Premium Feel"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What would make this account feel more premium or unlocked?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "premium" in content or "unlocked" in content or "room to breathe" in content
    assert "bone voyage" in content or "support" in content or "usable" in content


@pytest.mark.asyncio
async def test_chat_can_identify_tempting_task_that_is_too_early(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"TMP{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Too Early"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What kind of task is secretly too early even if it looks tempting?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "too early" in content or "tempting" in content or "awkward" in content
    assert "bone voyage" in content or "combat" in content or "support" in content


@pytest.mark.asyncio
async def test_chat_can_identify_progress_that_makes_account_less_awkward(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"AWK{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Less Awkward"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What kind of progress would make this account feel less awkward and more complete?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "less awkward" in content or "more complete" in content or "friction" in content
    assert "bone voyage" in content or "combat" in content or "cleanup" in content


@pytest.mark.asyncio
async def test_chat_can_identify_progress_that_makes_account_more_prestigious(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"PRE{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Prestige"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What kind of progress would make this account feel more prestigious?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "prestigious" in content or "distinguished" in content or "finished kind of status" in content
    assert "bone voyage" in content or "support" in content or "combat" in content


@pytest.mark.asyncio
async def test_chat_can_identify_flashy_low_value_task(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"FLA{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Flashy Value"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What kind of task is flashy but low real value right now?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "flashy" in content or "low real value" in content or "spectacle" in content
    assert "bone voyage" in content or "combat" in content or "support" in content


@pytest.mark.asyncio
async def test_chat_can_identify_improvement_that_transforms_account_most(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"TRN{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Transform Account"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What one improvement would make the account feel most transformed?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "transform" in content or "transformed" in content or "changes how the whole account feels" in content
    assert "bone voyage" in content or "combat" in content or "convenience unlock" in content


@pytest.mark.asyncio
async def test_chat_can_identify_progress_that_makes_account_more_self_sufficient(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"SEL{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Self Sufficient"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What kind of progress would make this account feel more self sufficient?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "self-sufficient" in content or "dependable" in content or "capable on its own" in content
    assert "bone voyage" in content or "support" in content or "combat" in content


@pytest.mark.asyncio
async def test_chat_can_identify_habit_that_makes_account_feel_more_premium_over_month(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"MON{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Premium Month"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What kind of habit would make the account feel more premium over a month?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "premium" in content or "polished" in content or "finished" in content
    assert "bone voyage" in content or "support" in content or "combat" in content


@pytest.mark.asyncio
async def test_chat_can_identify_unlock_that_makes_account_feel_more_open(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"OPN{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "More Open"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What one unlock would make the account feel dramatically more open?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "open" in content or "wider" in content or "more room" in content
    assert "bone voyage" in content or "support" in content or "combat" in content


@pytest.mark.asyncio
async def test_chat_can_identify_progress_that_feels_elite_without_being_joyless(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"ELT{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Elite Without Joyless"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What kind of progress would make the account feel more elite without becoming joyless?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "elite" in content or "prestige" in content or "without becoming joyless" in content
    assert "bone voyage" in content or "support" in content or "combat" in content


@pytest.mark.asyncio
async def test_chat_can_identify_habit_that_quietly_wastes_account_potential(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"WST{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Quiet Waste"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What kind of habit would quietly waste the account's potential?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "waste" in content or "almost-ready" in content or "delay" in content
    assert "bone voyage" in content or "support" in content or "combat" in content


@pytest.mark.asyncio
async def test_chat_can_identify_improvement_that_makes_account_future_proof(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"FTR{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Future Proof"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What one improvement would make the account feel dramatically more future proof?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "future-proof" in content or "future proof" in content or "sturdier foundation" in content
    assert "bone voyage" in content or "support" in content or "combat" in content


@pytest.mark.asyncio
async def test_chat_can_identify_change_that_reduces_friction_across_account(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"FRI{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Reduce Friction"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What one change would reduce friction across the whole account?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "reduce friction" in content or "convenience layer" in content or "easier" in content
    assert "bone voyage" in content or "support" in content or "drag" in content


@pytest.mark.asyncio
async def test_chat_can_identify_what_is_quietly_carrying_account(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"CRY{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Quietly Carrying"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What part of this account is quietly carrying everything?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "quietly carrying" in content or "support thread" in content or "real weight" in content
    assert "bone voyage" in content or "combat" in content or "momentum" in content


@pytest.mark.asyncio
async def test_chat_can_identify_what_makes_account_more_legendary_without_tedium(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"LEG{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Legendary Feel"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What would make this account feel more legendary without becoming tedious?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "legendary" in content or "heroic" in content or "prestige" in content
    assert "bone voyage" in content or "combat" in content or "support" in content


@pytest.mark.asyncio
async def test_chat_can_identify_progress_that_makes_account_calmer_to_manage(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"CAL{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Calmer Account"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What kind of progress would make this account feel calmer and easier to manage?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "calmer" in content or "easier to manage" in content or "mental load" in content
    assert "bone voyage" in content or "drag" in content or "support" in content


@pytest.mark.asyncio
async def test_chat_can_identify_impressive_but_underperforming_part_of_account(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"IMP{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Looks Impressive"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What part of this account looks impressive but is doing less than it seems?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "impressive" in content or "pulling enough weight" in content or "not fully online" in content
    assert "combat" in content or "bone voyage" in content or "utility" in content


@pytest.mark.asyncio
async def test_chat_can_identify_unlock_that_makes_account_more_effortless(
    client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_enriched_account_summary(rsn: str) -> dict[str, object]:
    ...
    account_response = await client.post("/api/accounts", json={"rsn": f"EFT{uuid4().hex[:8]}"})
    account_id = account_response.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync")
    await client.patch(
        f"/api/accounts/{account_id}/progress",
    ...
    session_response = await client.post("/api/chat/sessions", json={"title": "Effortless Day To Day"})

    response = await client.post(
        f"/api/chat/sessions/{session_response.json()['id']}/messages",
        json={"content": "What one unlock would make the account feel more effortless day to day?"},
    )

    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "effortless" in content or "day to day" in content or "quality of life" in content
    assert "bone voyage" in content or "drag" in content or "setup" in content
...
async def test_chat_uses_companion_quest_state_for_unlock_reasoning(client: AsyncClient) -> None:
    ...
    await client.post(
        "/api/goals",
        cookies=cookies,
        json={"title": "Quest Cape", "goal_type": "quest cape", "target_account_rsn": "AwareRsn"},
    )
...
async def test_gear_recommendations_see_companion_notable_items(client: AsyncClient) -> None:
    ...
    item_names = [item["item_name"].lower() for item in response.json()["recommendations"]]
    assert "abyssal whip" not in item_names
...
async def test_chat_why_ranked_answer_uses_saved_action_context(client: AsyncClient) -> None:
    ...
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionWhy"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Why"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionWhy"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    assert "ranked highly" in content
    ...
    assert "bank state is missing" in content


@pytest.mark.asyncio
async def test_chat_why_ranked_answer_uses_score_breakdown_from_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Score User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionScore"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Score"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 74,
                "priority": "medium",
                "target": {"quest_id": "bone-voyage", "account_rsn": "ActionScore"},
                "blockers": [],
                "supporting_data": {
                    "score_breakdown": {
                        "base_score": 92,
    ...
    assert response.status_code == 201
    content = response.json()["assistant_message"]["content"].lower()
    assert "starts at 92" in content
    assert "adjusts by -18" in content
    assert "lands at 74" in content
    assert "already-known unlock penalty" in content


@pytest.mark.asyncio
async def test_chat_follow_up_uses_saved_action_context_for_recommendation_change_question(
    client: AsyncClient,
) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Follow Up User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionFollow"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Follow Up"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionFollow"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    follow_up_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What would make this recommendation change?"},
    )

    assert follow_up_response.status_code == 201
    content = follow_up_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "recommendation would change" in content
    assert "bank state" in content


@pytest.mark.asyncio
async def test_chat_focus_summary_recaps_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Recap User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionRecap"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Recap"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionRecap"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    assert recap_response.status_code == 201
    content = recap_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
...
@pytest.mark.asyncio
async def test_chat_blocker_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Blocker User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionBlock"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Blocker"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionBlock"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    content = blocker_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "bank state missing" in content
    assert "bank state is missing" in content
...
@pytest.mark.asyncio
async def test_chat_next_step_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Next User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionNext"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Next"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionNext"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    next_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What comes after this?"},
    )

    assert next_response.status_code == 201
    content = next_response.json()["assistant_message"]["content"].lower()
    assert "after train magic" in content
    assert "high alchemy" in content
    assert "bank state missing" in content
    assert "fresh sync" in content


@pytest.mark.asyncio
async def test_chat_checklist_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Checklist User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionList"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Checklist"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionList"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    checklist_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "Can you turn this recommendation into a checklist?"},
    )

    assert checklist_response.status_code == 201
    content = checklist_response.json()["assistant_message"]["content"].lower()
...
@pytest.mark.asyncio
async def test_chat_success_check_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Success User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionProof"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Success"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionProof"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    success_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "How will I know this recommendation is working?"},
    )

    assert success_response.status_code == 201
    content = success_response.json()["assistant_message"]["content"].lower()
...
@pytest.mark.asyncio
async def test_chat_low_gp_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Low GP User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionCash"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Low GP"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionCash"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    gp_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What if I don't have much GP for this recommendation?"},
    )

    assert gp_response.status_code == 201
    content = gp_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "gp" in content or "money" in content
    assert "cautious" in content
    assert "exact cost" in content or "profit" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content


@pytest.mark.asyncio
async def test_chat_budget_action_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Budget User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionBudget"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Budget"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionBudget"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    budget_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What's the budget version of this recommendation?"},
    )

    assert budget_response.status_code == 201
    content = budget_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
...
@pytest.mark.asyncio
async def test_chat_ironman_action_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Iron User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionIron"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Iron"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionIron"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    iron_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What if I'm an ironman for this recommendation?"},
    )

    assert iron_response.status_code == 201
    content = iron_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "ironman" in content
    assert "self-sufficient" in content or "self-supplied" in content
    assert "high alchemy" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content
...
@pytest.mark.asyncio
async def test_chat_hardcore_action_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Hardcore User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionHard"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Hardcore"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionHard"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    hardcore_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What if I'm a hardcore ironman for this recommendation?"},
    )

    assert hardcore_response.status_code == 201
    content = hardcore_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "hardcore" in content
    assert "safe" in content or "risk" in content
    assert "death" in content or "danger" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content


@pytest.mark.asyncio
async def test_chat_ultimate_ironman_action_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action UIM User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionUim"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action UIM"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionUim"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    uim_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What if I'm UIM for this recommendation?"},
    )

    assert uim_response.status_code == 201
    content = uim_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "ultimate ironman" in content or "uim" in content
    assert "inventory" in content
    assert "storage" in content or "setup" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content


@pytest.mark.asyncio
async def test_chat_group_ironman_action_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action GIM User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionGim"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action GIM"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionGim"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    gim_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What if I'm a group ironman for this recommendation?"},
    )

    assert gim_response.status_code == 201
    content = gim_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "group ironman" in content or "gim" in content
    assert "team" in content or "group" in content
    assert "supply" in content or "shared" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content


@pytest.mark.asyncio
async def test_chat_pure_action_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Pure User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionPure"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Pure"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionPure"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    pure_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What if I'm a 1 defence pure for this recommendation?"},
    )

    assert pure_response.status_code == 201
    content = pure_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
...
@pytest.mark.asyncio
async def test_chat_zerker_action_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Zerker User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionZerk"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Zerker"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionZerk"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    zerker_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What if I'm a zerker for this recommendation?"},
    )

    assert zerker_response.status_code == 201
    content = zerker_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "zerker" in content or "berserker" in content
    assert "defence" in content
    assert "bracket" in content or "build" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content
...
@pytest.mark.asyncio
async def test_chat_missing_requirements_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Requirements User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionReq"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Requirements"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionReq"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    requirements_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What if I don't have the requirements for this recommendation?"},
    )

    assert requirements_response.status_code == 201
    content = requirements_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "requirement" in content
    assert "blocker" in content
    assert "smaller" in content or "fallback" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content
...
async def test_chat_prep_answer_uses_saved_action_context(client: AsyncClient) -> None:
    ...
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionPrep"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Prep"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionPrep"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    prep_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What should I prep before doing this recommendation?"},
    )

    assert prep_response.status_code == 201
    content = prep_response.json()["assistant_message"]["content"].lower()
...
@pytest.mark.asyncio
async def test_chat_stop_condition_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Stop User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionStop"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Stop"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionStop"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    stop_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "When should I stop doing this recommendation and switch away?"},
    )

    assert stop_response.status_code == 201
    content = stop_response.json()["assistant_message"]["content"].lower()
    assert "stop" in content
    assert "train magic" in content
    assert "fresh sync" in content
    assert "switch" in content or "change lanes" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content


@pytest.mark.asyncio
async def test_chat_tracking_signal_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Tracking User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionTrack"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Tracking"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionTrack"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    tracking_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What should I track while doing this recommendation?"},
    )

    assert tracking_response.status_code == 201
    content = tracking_response.json()["assistant_message"]["content"].lower()
...
@pytest.mark.asyncio
async def test_chat_bank_uncertainty_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Bank User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionBank"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Bank"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionBank"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    bank_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "Can I still do this recommendation without bank sync?"},
    )

    assert bank_response.status_code == 201
    content = bank_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "bank sync" in content or "bank state" in content
    assert "cautious" in content or "safe" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content


@pytest.mark.asyncio
async def test_chat_blocker_fallback_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Fallback User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionFall"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Fallback"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionFall"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    fallback_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What should I do if I can't clear this blocker yet?"},
    )

    assert fallback_response.status_code == 201
    content = fallback_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "bank state missing" in content
    assert "fallback" in content or "smaller" in content or "lighter" in content
    assert "fresh sync" in content
    assert "wealth assumptions" in content


@pytest.mark.asyncio
async def test_chat_alternative_path_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Alternative User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionAlt"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Alternative"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionAlt"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    alternative_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What is an alternative if I don't want to do this exact method?"},
    )

    assert alternative_response.status_code == 201
    content = alternative_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
...
@pytest.mark.asyncio
async def test_chat_urgency_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Urgency User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionUrg"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Urgency"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionUrg"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    urgency_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "Is this urgent or can it wait?"},
    )

    assert urgency_response.status_code == 201
    content = urgency_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "critical" in content
...
@pytest.mark.asyncio
async def test_chat_worth_time_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Worth User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionWorth"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Worth"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionWorth"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    worth_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "Is this actually worth my time?"},
    )

    assert worth_response.status_code == 201
    content = worth_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "worth" in content
    assert "critical" in content
    assert "91" in content
...
@pytest.mark.asyncio
async def test_chat_account_fit_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Fit User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionFit"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Fit"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionFit"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    fit_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "Does this fit my account right now?"},
    )

    assert fit_response.status_code == 201
    content = fit_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "actionfit" in content
    assert "magic" in content
    assert "critical" in content
    assert "91" in content
    assert "bank state missing" in content
...
@pytest.mark.asyncio
async def test_chat_missing_data_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Missing Data User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionData"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Missing Data"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionData"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    data_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What data do you still need to be more confident about this?"},
    )

    assert data_response.status_code == 201
    content = data_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "missing" in content or "need" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content
    assert "fresh sync" in content
...
@pytest.mark.asyncio
async def test_chat_ignore_consequence_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Ignore User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionSkip"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Ignore"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionSkip"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    ignore_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What happens if I ignore this recommendation?"},
    )

    assert ignore_response.status_code == 201
    content = ignore_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "ignore" in content or "skip" in content
    assert "critical" in content
    assert "91" in content
    assert "bank state missing" in content
    assert "fresh sync" in content
...
@pytest.mark.asyncio
async def test_chat_report_back_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Report User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionRpt"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Report"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionRpt"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    report_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What should I report back after doing this recommendation?"},
    )

    assert report_response.status_code == 201
    content = report_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
...
@pytest.mark.asyncio
async def test_chat_review_window_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Window User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionWin"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Window"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionWin"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    window_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "How long should I try this before checking back?"},
    )

    assert window_response.status_code == 201
    content = window_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "one focused session" in content or "short session" in content
    assert "fresh sync" in content
    assert "magic" in content
    assert "bank state missing" in content
    assert "wealth assumptions" in content
...
@pytest.mark.asyncio
async def test_chat_change_mind_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Change Mind User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionMind"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Change Mind"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionMind"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    change_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What would make you change your mind about this recommendation?"},
    )
...
@pytest.mark.asyncio
async def test_chat_action_evidence_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Evidence User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionProof"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Evidence"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    evidence_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What data are you using for this recommendation?"},
    )

    assert evidence_response.status_code == 201
    content = evidence_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
...
@pytest.mark.asyncio
async def test_chat_afk_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action AFK User"})
    cookies = auth.cookies
    await client.patch("/api/profile", cookies=cookies, json={"prefers_afk_methods": True})
    account = await client.post("/api/accounts", json={"rsn": "ActionAfk"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action AFK"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionAfk"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
    ...
    afk_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "I want AFK progress. Can I do that with this recommendation?"},
    )

    assert afk_response.status_code == 201
    content = afk_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "high alchemy" in content
    assert "afk" in content or "low-attention" in content
    assert "bank state missing" in content


@pytest.mark.asyncio
async def test_chat_deprioritize_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Deprioritize User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionDrop"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Deprioritize"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionDrop"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    deprioritize_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What would you deprioritize this week?"},
    )

    assert deprioritize_response.status_code == 201
    content = deprioritize_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "deprioritize" in content
    assert "new lane" in content or "distraction" in content
    assert "bank state missing" in content
...
@pytest.mark.asyncio
async def test_chat_lower_effort_answer_uses_saved_action_context(client: AsyncClient) -> None:
    auth = await client.post("/api/auth/dev-login", json={"display_name": "Action Lower Effort User"})
    cookies = auth.cookies
    account = await client.post("/api/accounts", json={"rsn": "ActionEasy"}, cookies=cookies)
    account_id = account.json()["id"]
    await client.post(f"/api/accounts/{account_id}/sync", cookies=cookies)
    session = await client.post("/api/chat/sessions", json={"title": "Action Lower Effort"}, cookies=cookies)
    session_id = session.json()["id"]
    ...
    first_response = await client.post(
        ...
        json={
            ...
            "action_context": {
                ...
                "score": 91,
                "priority": "critical",
                "target": {"skill": "magic", "account_rsn": "ActionEasy"},
                "blockers": ["bank state missing"],
                "supporting_data": {
                    "recommended_skill": "magic",
                    "readiness_warning": "Bank state is missing, so do not make exact wealth assumptions.",
    ...
    lower_effort_response = await client.post(
        f"/api/chat/sessions/{session_id}/messages",
        cookies=cookies,
        json={"content": "What should I do if I want something lower effort but still useful for this recommendation?"},
    )

    assert lower_effort_response.status_code == 201
    content = lower_effort_response.json()["assistant_message"]["content"].lower()
    assert "train magic" in content
    assert "lower effort" in content
    assert "useful" in content
    assert "smallest" in content or "lighter" in content
    assert "bank state missing" in content
